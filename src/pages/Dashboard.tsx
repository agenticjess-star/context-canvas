...
import { Plus, Eye, Clock, Trash2, ExternalLink, Copy, Check, Loader2, Layers, FileText } from 'lucide-react';
...
const Dashboard = () => {
  ...
  return (
    <AppLayout activeTab="canvases" isGuest={isGuest} onNewCanvas={atLimit ? undefined : () => navigate('/dashboard/new')}>
      {atLimit && !isGuest && (
        <div className="mb-6 text-sm rounded-xl border border-primary/30 bg-primary/5 text-primary px-4 py-3 flex items-center justify-between">
          <span>You've reached your {planName} plan limit ({limit} canvases).</span>
          <button className="font-medium underline underline-offset-2" onClick={() => navigate('/pricing')}>Upgrade</button>
        </div>
      )}
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{canvases.length} of {limit} used · {planName} plan</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6 max-w-xs">
            Create your first context canvas to start sharing AI-ready content.
          </p>
          {!atLimit && (
            <Button className="rounded-full px-6 gap-2" onClick={() => navigate('/dashboard/new')}>
          ...
          <AnimatePresence>
            {canvases.map((c) => (
              <motion.div
                ...
                exit={{ opacity: 0, y: -8 }}
                className="group flex items-center gap-4 p-4 rounded-xl bg-card border border-border hover:border-primary/15 transition-colors"
              >
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => window.open(getCanvasUrl(c), '_blank')} className="p-2 rounded-lg text-muted-foreground hover:text-foreground transition-colors" title="Open">
                    <ExternalLink className="h-3.5 w-3.5" />
                  </button>
                  <button onClick={() => { navigator.clipboard.writeText(getCanvasUrl(c)); setCopiedId(c.id); }} className="p-2 rounded-lg text-muted-foreground hover:text-foreground transition-colors" title="Copy link">
                    {copiedId === c.id ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
                  </button>
                  <button onClick={async () => { await getDataProvider(isGuest).deleteCanvas(c.id); setCanvases(x => x.filter(y => y.id !== c.id)); }} className="p-2 rounded-lg text-muted-foreground hover:text-destructive transition-colors" title="Delete">
