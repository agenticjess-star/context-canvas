    ...
    if (type === "text" && tempText.trim()) {
      setSources((s) => [
        ...s,
        {
          ...
          type: "text",
          label: tempText.slice(0, 60) + (tempText.length > 60 ? "…" : ""),
          content: tempText,
        },
      ]);
      setTempText("");
  ...
  const handleGenerate = () => {
    if (sources.length === 0) {
      toast({
        ...
        description: "Add at least one source to generate a context page.",
        variant: "destructive",
      });
      return;
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8 lg:gap-12">
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-3">
              <input
                placeholder="Untitled context page"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                className="w-full text-2xl sm:text-3xl font-bold bg-transparent border-none outline-none placeholder:text-muted-foreground/30 tracking-tight"
              />
              <input
                ...
                {sources.map((src) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="group flex items-center gap-3 p-4 rounded-xl bg-card border border-border hover:border-primary/15 transition-all duration-200"
                    >
              ...
              {sources.length === 0 && !addingType && (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-xs text-muted-foreground/60 max-w-xs">
                    Add text notes, paste URLs, or drag files here to build your context page.
                  </p>
                </div>
              )}
            </div>

            {/* ── Add source inline panels ── */}
            <AnimatePresence>
              {addingType === "text" && (
                <motion.div
                ...
                >
                  <div className="space-y-3 p-5 rounded-xl border border-primary/20 bg-card">
                    ...
                    <Textarea
                      placeholder="Paste your research notes, meeting summaries, requirements, specs…"
                      value={tempText}
                ...
                >
                  <div className="space-y-3 p-5 rounded-xl border border-primary/20 bg-card">
                    ...
                    <p className="text-xs text-muted-foreground">
                      We'll extract the page content and optimize it for AI readability.
                    </p>
                    <div className="flex gap-2 justify-end">
                      <Button
                        size="sm"
                    ...
                    <label className="flex flex-col items-center justify-center gap-3 py-10 rounded-xl border-2 border-dashed border-border cursor-pointer hover:border-primary/30 hover:bg-primary/[0.02] transition-all">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          PDF, PNG, JPG, TXT, MD — up to 20 MB
                        </p>
                      </div>
                      <input
                        type="file"
                ...
                {(["text", "url", "file"] as SourceType[]).map((type) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-xl border border-border bg-card hover:border-primary/20 hover:bg-primary/[0.02] transition-all duration-200"
                    >
            ...
            <div className="sticky top-20 space-y-5">
                ...
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    ...
                    <p className="text-sm font-medium">
                      {title || <span className="text-muted-foreground/40">Untitled</span>}
                    </p>
                  </div>
              ...
              <div className="rounded-2xl border border-border bg-muted/30 p-5 space-y-3">
                ...
                <ul className="space-y-2 text-xs text-muted-foreground leading-relaxed">
                  ...
                  <li className="flex gap-2">
                    <span className="text-primary mt-0.5">•</span>
                    URLs are automatically scraped for content
                  </li>
                  <li className="flex gap-2">
                    <span className="text-primary mt-0.5">•</span>
                    More sources = richer context for AI tools
                  </li>
                </ul>
              </div>
            </div>
