      ...
      <section className="relative pt-16 sm:pt-24 pb-32 px-6">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <motion.p initial="hidden" animate="visible" variants={fadeUp} custom={2} className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-10 leading-relaxed">
            Drop links, notes, and files into EasyContext. Get a single optimized URL
        ...
        <div className="relative max-w-5xl mx-auto mt-20">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.5, ease: [0.25, 0.4, 0.25, 1] }} className="relative mx-auto">
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.9 }} className="absolute -left-4 sm:-left-10 top-8 bg-card border border-border rounded-xl p-3 sm:p-4 shadow-[0_8px_32px_hsl(228_20%_10%/0.08)] hidden sm:block">
      ...
      <section className="px-6 py-24 bg-muted/30">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
            {[
              { step: "01", icon: FileText, title: "Add your sources", desc: "Paste text, drop links, or upload PDFs and images. Mix and match — we handle all of it." },
              { step: "02", icon: Zap, title: "We optimize it", desc: "Content is cleaned, structured, and formatted for maximum AI comprehension. No fluff." },
              { step: "03", icon: Globe, title: "Share one URL", desc: "Paste the link into ChatGPT, Claude, or any AI. They get full context instantly." },
            ].map((item, i) => (
              <motion.div key={item.step} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={fadeUp} custom={i} className="relative bg-card border border-border rounded-2xl p-7 group hover:border-primary/20 transition-colors duration-300">
                <span className="absolute top-6 right-6 text-sm font-bold text-muted-foreground/20 group-hover:text-primary/20 transition-colors">{item.step}</span>
      ...
      <section className="px-6 py-24">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-balance">Built for people who use AI seriously</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {[
              ...
              <motion.div key={item.title} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={fadeUp} custom={i} className="flex gap-4 p-6 rounded-2xl border border-border bg-card hover:border-primary/15 transition-colors duration-300">
